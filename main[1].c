...
/* Function used in encrypt */
unsigned int F(unsigned int mes, unsigned int sbox[][256])
{
   ...
   mes = (((sbox[0][a] + sbox[1][b]) ^ sbox[2][c]) + sbox[3][d]);
   return mes;
}

/* The encryption part of the blowfish algorithm. The message is taken in 2 halves (m[0] and m[1]) and passed by reference in the m array */
void encrypt(unsigned int m[], unsigned int parr[], unsigned int sbox[][256])
{
   unsigned int r = m[1];
   unsigned int l = m[0];
   unsigned int L = 0;
...
/* Takes a 16 character key as a command line argument and takes a message (of 16 characters or less) from standard input */
int main(int argc, char* argv[])
{
   /* the P array is 18 32-bit integers */
   unsigned int parray[] = {
   ...
   /* Each S array is 256 32-bit integers.  There are 4 of them. */
   unsigned int sbox[][256] = {{ 0xd1310ba6L, 0x98dfb5acL, 0x2ffd72dbL, 0xd01adfb7L, 0xb8e1afedL, 0x6a267e96L,
   ...
   /* Fill all array values with zeroes or null characters */
   for (int j = 0; j < 5; j++)
   {
      left[j] = '\0';
      right[j] = '\0';
   ...
   /* Store each half of the key in k0 and k1 and convert both to long*/
   for (int i = 0; i < 8; i++)
   {
      k0[i]  = argv[1][i];
      k1[i]  = argv[1][i + 8]; 
   } 
   key0 = strtol(k0, NULL, 16);
   key1 = strtol(k1, NULL, 16);
 
   /* Initialize the parray */ 
   for (int i = 0; i < 18; i++)
   {
      if (i%2 == 0)
         parray[i] = parray[i] ^ key0;
      else 
         parray[i] = parray[i] ^ key1; 
   }
   for (int i = 0; i < 17; i++)
   {
      if (i % 2 == 0)
      { 
         encrypt(mes, parray, sbox);     
         parray[i] = mes[0];
         parray[i+1] = mes[1];
     }
   }
   ...
   /* Get the message from standard input and store the initial left and right halves */
   scanf("%s", message);

   for (int i = 0; i < 4; i++)
   {
      t = 1;
      for (int j = 3; j > i; j--) t = t * 100; /* Find the appropriate exponent */
      left[i] = message[i];
      right[i] = message[i + 4];
      sprintf(temp, "%02x", left[i]);
      sprintf(temp2, "%02x", right[i]);
      mes[0] += strtol(temp, NULL, 10) * t;
      mes[1] += strtol(temp2, NULL, 10) * t; /* mes stores decimal integers that should be interpreted as hex integers. */
   }

   /* Make sure the value stored in mes is treated as a hex value instead of a decimal value */
   sprintf(message, "%d", mes[0]); /* message used as a temporary variable */
   mes[0] = strtol(message, NULL, 16);
   sprintf(message, "%d", mes[1]);
   mes[1] = strtol(message, NULL, 16);
